...
| 打开任意一部动画的剧集列表 | `GET https://anime1.me/?cat={id}`，随后**自动**跟随页面内“上一頁”分页链接连续请求 | **N**（未命中缓存时；N = 该分类下 WordPress 分页页数，逐页顺序请求直至无下一页）；**0**（命中进程内缓存时） | 全部分页成功后按 `动画 id + 分类地址` 存入 `episodeCache`（进程内 Map），TTL 10 分钟；TTL 内重复打开同一动画直接命中缓存，不发请求（见第 3 节） |
...
这是全部请求中唯一会随内容量线性增长的部分，值得重点关注；缓存未命中时的行为如下：

```text
用户点击一部动画
  → AnimeViewModel.openAnime()
  → 命中未过期缓存？ → 直接展示缓存剧集，0 次请求，不启动 episode Job
  → 未命中/已过期：
...
- **短期进程内缓存**：全部分页成功后，合并去重的最终列表写入 `AnimeViewModel.episodeCache`，key 为 `动画 id + 分类地址`，TTL 为 10 分钟（`EPISODE_CACHE_TTL_MILLIS`，按 `SystemClock.elapsedRealtime()` 单调计时）。TTL 内返回动画列表后再次打开同一部动画不会产生任何请求；TTL 过期后重新顺序请求全部 N 页并用新结果替换缓存。
- **只缓存完整成功的结果**：任一分页失败、请求被取消，或结果返回时用户已切换到别的动画，都不会写入缓存，也不会覆盖已有缓存；下次打开仍会重新请求。
- **缓存仅存在于当前进程**：不写文件、`SharedPreferences` 或数据库，进程重启后自然清空。
- **缓存包含播放签名**：`Episode.apiRequest` 会随剧集列表一起被缓存，10 分钟的短 TTL 用于降低签名陈旧的风险；若缓存中的签名已失效，播放重试仍会刷新该剧集所在的单页并取得新签名。
...
- **按 key 缓存 + TTL**：剧集列表分页（按 `动画 id + 分类地址` 缓存完整成功的结果，10 分钟内命中不再请求，过期后全量重取）。
- **每次触发都重新请求，不缓存**：播放地址解析（每次点击播放都重新请求）。
...
7. 返回，10 分钟内再次打开步骤 2 中同一部动画（命中缓存）      → 0
────────────────────────────────────────────────────────
合计                                                    → 13 次元数据请求
```

其中第 7 步在增加剧集缓存前需要 5 次请求；反复比较剧集的场景下，这部分重复请求现在只在缓存过期（10 分钟）后才会再次产生。

## 7. 已识别的风险点

- **剧集分页无节流**：长篇动画分页越多，首次打开（或缓存过期后重新打开）该动画产生的连续 GET 越多；请求间没有延时，是短时间内对服务器发起请求最密集的路径。
- **缓存的播放签名可能过期**：剧集缓存同时缓存了播放签名，最长可陈旧 10 分钟；此时播放会失败，需要用户点击“重试”刷新单页签名后才能恢复。
